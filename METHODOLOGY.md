...
**CISA Advisories (AA-numbered): high-yield, structured.** Joint Cybersecurity Advisories often include explicit ATT&CK technique tables that CISA themselves prepared. The ingester extracts mentions from these tables with `extraction_method = 'cisa_attack_table'` and confidence 1.0, distinct from regex-extracted mentions. The regex extractor runs over the body prose *with the ATT&CK tables removed*, so the two passes are independent: a technique reported by both methods genuinely appears in CISA's structured mapping *and* in the advisory narrative — it is not the regex pass re-reading the same table. Advisories without explicit tables fall through to the regex extractor over their full prose.
